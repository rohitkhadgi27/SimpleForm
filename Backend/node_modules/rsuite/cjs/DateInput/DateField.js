...
const patternMap = exports.patternMap = {
  y: 'year',
  M: 'month',
  d: 'day',
  H: 'hour',
...
class DateField extends Object {
  constructor(format, value) {
    ...
    if (value && (0, _isValid.isValid)(value)) {
...
const useDateField = (format, localize, date) => {
  const [dateField, dispatch] = (0, _react.useReducer)((state, action) => {
    switch (action.type) {
      case 'setYear':
        return {
          ...state,
  ...
  const validFieldValue = (type, value) => {
    var _format$match;
    let isValid = true;
    (_format$match = format.match(new RegExp('([y|d|M|H|h|m|s])+', 'ig'))) === null || _format$match === void 0 || _format$match.forEach(pattern => {
  ...
  const toDate = (type, value) => {
    ...
    const date = new Date(year || 0, typeof month === 'number' ? Math.max(month - 1, 0) : 0,
